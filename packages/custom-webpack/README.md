...
```typescript
(options: TargetOptions) => (indexHtmlContent: string) => Promise<string>|Observable<string>;
```
or, in other words, the function receives `TargetOptions` and returns a function which, in turn, returns a function that receives index html content and returns a Promise/Observable with a new content.
...
```
It is useful when you'd like to transform your `index.html` according to the build options. 
## Example
`angular.json`:
```js
"architect": {
