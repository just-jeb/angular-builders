...
describe('CustomWebpackBuilder test', () => {
	...
	it('Should load webpack.config.js if no path specified', () => {
		...
		expect(mergeConfigs).toHaveBeenCalledWith(baseWebpackConfig, customWebpackConfig, undefined, undefined);
	});
	...
	it('Should pass on merge strategies', () => {
		...
		expect(mergeConfigs).toHaveBeenCalledWith(baseWebpackConfig, customWebpackConfig, mergeStrategies, undefined);
	});
	...
	it('Should pass on replaceDuplicatePlugins flag', () => {
		...
		expect(mergeConfigs).toHaveBeenCalledWith(baseWebpackConfig, customWebpackConfig, undefined, true);
	});
