...
### Zone.js Setup Evolution

Four iterations of zone.js setup exist in git history (Source: code investigation, 2026-02-16):

1. **Pre-v16** -- `import 'jest-preset-angular'` plus global mocks
2. **v16-v18** -- Switched to `import 'jest-preset-angular/setup-jest'`
3. **v19-v20** -- Explicit `setupZoneTestEnv()` from `jest-preset-angular/setup-env/zone`, always zone-based
4. **v21+** -- Split into `setup-zoneless.ts` and `setup-zone.ts` to support Angular 21's zoneless default

The zoneless/zone split was driven by Angular's shift to zoneless as the default change detection strategy. Users upgrading to v21 must set `zoneless: false` if their app still uses zone.js.

### Global Mock History

Historical progression (Source: code investigation, 2026-02-16):

- **Pre-v16:** 6+ mocks (styleTransform, getComputedStyle, doctype, localStorage, sessionStorage, matchMedia, etc.)
- **v16-v20:** 4 mocks (localStorage and sessionStorage removed -- Jest's jsdom provides them natively)
- **v21+:** Only `matchMedia` remains (styleTransform, getComputedStyle, doctype removed -- Jest 30's jsdom supports them natively)

The `matchMedia` mock was also fixed (issue #1983) to survive Jest's `resetMocks` option by switching from `jest.fn().mockImplementation()` to a regular function.
...
| "Global mocks include style transforms"                          | Since v21, `globalMocks` defaults to `["matchMedia"]` only. The `styleTransform`, `getComputedStyle`, and `doctype` mocks were removed because Jest 30's jsdom supports them natively. See "Global Mock History" above.                                                                                                                                                                                                                                                                                                                                                                                                                      |
...
| "Migrating from Karma is straightforward"                        | Six common mistakes during Karma-to-Jest migration (Source: code investigation, 2026-02-16): (1) Not updating `tsconfig.spec.json` to replace `jasmine` with `jest` in `types` and remove Karma `test.ts` from `files`. (2) Zone.js confusion post-v21 -- not setting `zoneless: false` when app still uses zone.js. (3) Not realizing `setupFilesAfterEnv` and `astTransformers` are concatenated (not replaced) during config merge. (4) Expecting old mocks (`styleTransform`, `getComputedStyle`, `doctype`) that were removed in v21. (5) `resetMocks` breaking matchMedia (issue #1983). (6) Not removing Karma deps and config files. |
