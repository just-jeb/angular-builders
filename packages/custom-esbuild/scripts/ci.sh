...
yarn test -c esm --no-watch # unit-test with esm custom config
yarn test -c cjs --no-watch # unit-test with cjs custom config
...
yarn test -c esm --no-watch # unit-test with esm custom config
yarn test -c cjs --no-watch # unit-test with cjs custom config
yarn test-ts -c tsEsm --no-watch # unit-test with TS custom config
yarn e2e # no custom config at all
yarn e2e -c esm # esm custom config
yarn e2e -c cjs # cjs custom config
